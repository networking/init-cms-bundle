...
let toggleActive = (e) => {
    ...
    let toggleUrl = Routing.generate('admin_networking_initcms_layoutblock_toggleActive', {id: id})
    axios.get(toggleUrl).then( (response) => {
        let layoutBlockHtml = document.getElementById('layoutBlockHtml'+id)
        if(response.data.active){
            layoutBlockHtml.classList.remove('opacity-25')
            el.classList.remove('ki-minus-circle')
            el.classList.remove('btn-light-danger')
            el.classList.add('ki-check-circle')
            el.classList.add('btn-light-success')
            return
        }

        layoutBlockHtml.classList.add('opacity-25')
        el.classList.add('ki-minus-circle')
        el.classList.add('btn-light-danger')
        el.classList.remove('ki-check-circle')
...
KTUtil.onDOMContentLoaded(function () {
    ...
    KTUtil.on(document.body,  '.toggle-active', 'click', function (e) {
        toggleActive(e)
    })
    initDropZone();
})