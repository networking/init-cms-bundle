...
var MediaSelect = function(id, title, updateUrl, formElementUrl) {
    var mediaModal = {
        dialog: false,
        dialogTitle: '',
        id: '',
        ...
        updateUrl: '',
        formElementUrl: '',
        init: function (id, title, updateUrl, formElementUrl) {
            this.id = id;
            this.title = title;
            this.updateUrl = updateUrl;
            this.formElementUrl = formElementUrl;
            this.field = $('#' + this.id);
            this.fieldWidget = $('#field_widget_' + this.id);
            var that = this;
            this.field.on('change', function () {
                that.logMessage('change', 'update label');
                that.fieldWidget.addClass('spinner spinner-lg spninner-left mr-20');
                that.fieldWidget.html('');
                if (that.field.val()) {
                    var url = that.updateUrl.replace('OBJECT_ID', jQuery(this).val());
                    axios.get(url, axiosConfig).then(function (response) {
                        that.fieldWidget.removeClass('spinner spinner-lg spninner-left mr-20').html(response.data);
                    });
                }else{
                    setTimeout(function (){
                        that.fieldWidget.removeClass('spinner spinner-lg spninner-left mr-20').html('<i class="la la-image icon-3x"></i>')
                    }, 500);
                }
    ...
    var mediaSelect = Object.create(mediaModal);
    mediaSelect.init(id, title, updateUrl, formElementUrl);
    return mediaSelect;
}