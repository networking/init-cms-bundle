...
class NetworkingInitCmsExtension extends Extension implements PrependExtensionInterface
{
    public function prepend(ContainerBuilder $container): void
    {
        ...
        foreach ($configs as $config) {
            ...
            if (isset($config['cache']) && true === $config['cache']['activate']) {
                $isCacheActive = true;
            }
        }
    ...
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        $loader->load('menus.php');
        $loader->load('services.php');
        $loader->load('validators.php');
        $loader->load('google_authenticator.php');
        $config['db_driver'] = 'orm';
        $loader->load(sprintf('doctrine_%s.php', $config['db_driver']));
        $loader->load(sprintf('ext_admin_%s.php', $config['db_driver']));
        $loader->load(sprintf('admin_%s.php', $config['db_driver']));

        if ($config['webauthn']['enabled']) {
            $loader->load('webauthn.php');
        }

        $loader->load('gedmo_doctrine_extensions.php');
    ...
    public function configureCache(array $config, ContainerBuilder $container)
    {
        ...
        if ($cacheService) {
            ...
            if (in_array(PageCacheInterface::class, $reflectionClass->getInterfaceNames())) {
