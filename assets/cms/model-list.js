...
class ModelList {
	constructor(element) {
		...
		KTUtil.on(
			this.element,
			`#field_dialog_${this.id}`,
			"hide.bs.modal",
			this.removeListeners.bind(this),
		)
	...
	initialize() {
		...
		if (this.removeLink) {
			this.removeLink.addEventListener(
				"click",
				this.removeSelectedElement.bind(this),
			)
		}
		if (this.addLink) {
	...
	createModal() {
		...
		this.dialog = new bootstrap.Modal(this.dialogContainer, {
			height: "auto",
			width: 650,
			show: false,
		})

		document.body.addEventListener("hidden.bs.modal", (_event) => {
	...
	addClickOnLinkListeners() {
		...
		this.selectModelEventId = KTUtil.on(
			this.dialogContainer,
			"a",
			"click",
			this.clickLinkInDialog.bind(this),
		)
	}
	addSearchListeners() {
		if (this.selectModelEventId !== "") {
		...
		this.searchModelEventId = KTUtil.on(
			this.dialogContainer,
			"form",
			"submit",
			this.searchModel.bind(this),
		)
	}
	addSubmitisteners() {
		...
		this.addModelEventId = KTUtil.on(
			this.dialogContainer,
			"form",
			"submit",
	...
	addPerPageListeners() {
		...
		this.perPageEventId = KTUtil.on(
			...
			(e) => {
				...
				axios.get(url.toString(), { ...axiosConfig }).then((response) => {
	...
	createListDialog(event) {
		event.preventDefault()
		const listURL = this.listLink.getAttribute("href")
		fetch(listURL, {
			method: "GET",
			headers: {
	...
	addDialog(event) {
		event.preventDefault()
		const addURL = this.addLink.getAttribute("href")
		fetch(addURL, {
			method: "GET",
			headers: {
				"X-Requested-With": "XMLHttpRequest",
	...
	submitModel(event) {
		...
		axios
			.post(form.getAttribute("action"), formData, { ...axiosConfig })
			.then((response) => {
			...
			.catch((error) => {
				...
				if (Object.hasOwn(data, "violations")) {
					data.violations.forEach((item) => {
						...
						if (field.closest(".tab-pane")) {
							const tabDiv = field.closest(".tab-pane")
							const tab = document.querySelector(`a[href="#${tabDiv.id}"]`)
	...
	reloadFormField(objectId) {
		axios.get(this.fieldReloadUrl, { ...axiosConfig }).then((response) => {
			...
			this.preview.innerHTML = shadow.querySelector(
				`#field_widget_${this.id}`,
			).innerHTML
			this.field = this.preview.querySelector(`#${this.id}`)

			const newElement = this.preview.querySelector(
				`#${this.id} [value="${objectId}"]`,
			)

			const fieldType = newElement.tagName.toLowerCase()
	...
	updatePreview() {
		...
		const url = this.preview.dataset.previewUrl.replace(
			"__objectId__",
			this.field.value,
		)
		fetch(url, {
	...
	removeSelectedElement(event) {
		...
		const elements = this.field.options
